...
export const getDevicesByClient = async (clientId: number): Promise<Device[]> => {
  const response = await axios.get(`${API_URL}/devices/client/${clientId}`);
  return response.data;
};
...
export const updateOrder = async (id: number, order: Order): Promise<Order> => {
  console.log(`API: Відправляю запит PUT на ${API_URL}/orders/${id} з даними:`, order);
  try {
    const response = await axios.put(`${API_URL}/orders/${id}`, order);
    console.log('API: Відповідь від сервера:', response.data);
    return response.data;
  } catch (error) {
    console.error('API: Помилка при оновленні замовлення:', error);
    throw error;
  }
};

export const updateOrderStatus = async (id: number, status: OrderStatus): Promise<Order> => {
  console.log(`API: Відправляю запит PATCH на ${API_URL}/orders/${id}/status з даними:`, { status });
  try {
    const response = await axios.patch(`${API_URL}/orders/${id}/status`, { status });
    console.log('API: Відповідь від сервера:', response.data);
    return response.data;
  } catch (error) {
    console.error('API: Помилка при зміні статусу:', error);
    throw error;
  }
};
...
export const getOrdersByDate = async (startDate: string, endDate: string): Promise<Order[]> => {
  const response = await axios.get(
    `${API_URL}/orders/date?startDate=${startDate}&endDate=${endDate}`
  );
  return response.data;
};
